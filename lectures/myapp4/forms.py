...
class UserForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if len(name) < 3:
            # не ломает код, а возвращает пользователю сообщение об ошибке
...
class ManyFieldsForm(forms.Form):
    ...
    # required - начальное состояние поля BooleanField. для остальных полей required определяет обязательность заполнения
    # required=True - обязательное заполнение поля
    is_active = forms.BooleanField(required=False)
    # initial параметр по умолчанию
...
class ManyFieldsFormWidget(forms.Form):
    # к отдельным полям можно применять любые стили css (в class)
    # placeholder - текст, к-й отобразится в пустом поле
    ...
    # вручную можно изменить тип виджета (type)
    birthdate = forms.DateField(initial=datetime.date.today,
